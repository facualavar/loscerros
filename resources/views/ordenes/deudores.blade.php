...
<div class="row">
    <div class="col-md-12">
        <div class="card">
        <div class="card-header"><h3>{{$seccion}}</h3></div>
            <div class="card-body">
                    ...
                    <div class="col-md-6">
                        <div class="card" style="min-height: 200px;">
                            ...
                            <div class="card-body">
                                <form action="">
                                    <div class="form-group">
                                        <input type="date" class="form-control" id="datepicker2" {{-- data-toggle="datetimepicker" data-target="#datepicker2"  --}}placeholder="fecha2">
                                    </div>
                                </form>
                            </div>
                        </div>
          ...
          success:function(resp){
              if(resp.status==1){
                ...
                data1=' <div class="table-responsive">'+
                        ...
                        '<tbody>'+
                        '<tr>'+
                            '<th style="with:10%">Fecha</th>'+
                            '<th style="with:10%">N° de Informe</th>'+
                            '<th style="with:20%">Paciente</th>'+
                            '<th style="with:20%">Obra Social</th>'+
                            '<th style="with:40%">Observaciones</th>';

                            jQuery.each(resp.data, function(index) {
                                data2+='<tr><td>'+resp.data[index].fecha+'</td>'+
                                        '<td>'+resp.data[index].idInformes+'</td>'+
                                        '<td>'+resp.data[index].paciente+'</td>'+
                                        '<td>'+resp.data[index].obra+'</td>'+
                                        '<td>'+resp.data[index].observaciones+'</td></tr>';
                            });
                            data3='</tbody>'+'</table></div>';
                    datos=data1+data2+data3;
                    $("#ordenes").empty().append(datos);
                    var url = '{{route("ordenespdf",["fecha1"=>"'fecha1'","fecha2"=>"'fecha2'","obra"=>"d"])}}';
